...
def render(supabase, user_id, xp):
    ...
    with c2:
        ...
        st.write("- **7天 高級語音試用**")
        if st.button("💰 付費解鎖中級"):
            database.upgrade_tier(supabase, user_id, "intermediate", energy_bonus=99, xp_bonus=20)
            st.balloons()
            st.success("升級成功！")
            st.rerun()
    ...
    with c3:
        ...
        st.write("- **優先體驗新功能**")
        if st.button("💰 付費解鎖高級"):
            database.upgrade_tier(supabase, user_id, "advanced", energy_bonus=599, xp_bonus=20)
            st.success("尊榮升級成功！")
            st.rerun()
