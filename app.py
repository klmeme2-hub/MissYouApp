...
from modules import ui, auth, database, audio, brain, config, gamification # 引入 gamification
from modules.tabs import tab_voice, tab_store, tab_persona, tab_memory, tab_config
import extra_streamlit_components as stx

# ==========================================
# 應用程式：EchoSoul (SaaS Beta 5.0 - 相似度引導版)
# ==========================================
...
if st.session_state.guest_data:
    ...
    # 訪客端也計算相似度，讓親友看到「完成度」
    sim_score, sim_hint, sim_gain = gamification.calculate_similarity(supabase, owner_id, role_name)
    ...
    if st.session_state.call_status == "ringing":
        ...
        ui.render_status_bar(tier, energy, 0, audio.get_tts_engine_type(profile), sim_score, sim_hint, sim_gain, is_guest=True)
...
# ------------------------------------------
    ...
    # 呼叫 Member View，並將 gamification 模組傳入 (如果 view 內部需要計算)
    # 不過為了保持 view 乾淨，我們可以在這裡算好再傳進去
    # 但 member view 結構比較大，建議直接在 member.py 引入 gamification
