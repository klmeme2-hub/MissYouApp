...
if "token" in params and not st.session_state.guest_data:
    ...
    disp_name = raw_token.split("_")[1] if "_" in raw_token else "朋友"
    
    data = database.validate_token(supabase, real_token)
    if data:
        st.session_state.guest_data = {
            'owner_id': data['user_id'], 'role': data['role'], 'display_name': disp_name
        }
        st.rerun()
    ...
    if st.session_state.call_status == "ringing":
        ...
        with c2:
            ...
            <div style='text-align:center; padding-top:50px;'>
                ...
                <p style='color:#CCC; font-size:20px;'>📞 來電中...</p>
...
# ------------------------------------------
    ...
    if not btn_disable and target_role not in allowed: st.info("🔒 需累積 20 XP 解鎖此角色")
    ...
    if st.session_state.show_invite:
        ...
        st.code(url)
        st.text_area("建議文案", value=f"欸！我做了一個AI分身超像的，點這個連結打電話給我：\n{url}")
